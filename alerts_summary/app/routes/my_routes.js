...
module.exports = function(app, db,callback) {
  ...
  // // console.log(account_name);
  // // const details = {'adopter_name':adopter_name};
  //   db.collection('mockdata').find({'username':username}).toArray(function (err, items){
  //     if (err) {
  //       res.send({'error':'An error has occurred'});
  //     } else {
  //       res.send(items);
...
//   // console.log(account_name);
//   //  const details = {};
//     db.collection('mockdata').updateOne({"username":"adopter111"},{$set:{"date":"2017-07-30"}},(err,items)=> {
//       if (err) {
